...
func Commit(repo *git.Repository, message string, parentRevs ...string) error {
	// The commit author.
	author, err := repo.DefaultSignature()
	if err != nil {
		// Failed to get default signature - must not be set
		fmt.Println("You don't have a username and email set for commits")
		fmt.Println("Please enter what username and email to use:")
		fmt.Print("Username: ")
		var username string
		...
		err = SetCreds(repo, username, email)
		if err != nil { return err }
